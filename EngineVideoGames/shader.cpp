...
void Shader::SetUniformMat4f(const std::string& name, const glm::mat4& matrix, int shaderIndx) {
	GLCall(glUniformMatrix4fv(GetUniformLocationWithShader(name, shaderIndx), 1, GL_FALSE, &matrix[0][0]));
}
...
int Shader::GetUniformLocationWithShader(const std::string& name, int shaderIndx)
{
	if (m_UniformLocationCache.find(name) != m_UniformLocationCache.end())
		return m_UniformLocationCache[name];
	...
	GLCall(int location = glGetUniformLocation(m_program, name.c_str()));
	if (location == -1)
		std::cout << "Warning: uniform '" << name << "' doesn't exist in shader " << shaderIndx << "!" << std::endl;
	m_UniformLocationCache[name] = location;
	return location;
}
